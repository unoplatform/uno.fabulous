        ...
        | Some options ->
            ...
            let result =
                ...
                |> Program.withWorkflow (fun workflow ->
                    { workflow with
                        optimize = UnoOptimizer.optimize })
                |> Program.withReadAssembliesConfiguration (fun configuration ->
                    { configuration with
                        isTypeResolvable = UnoConverters.isTypeResolvable
                        convertTypeName = UnoConverters.convertTypeName
                        tryGetStringRepresentationOfDefaultValue = UnoConverters.tryGetStringRepresentationOfDefaultValue })
