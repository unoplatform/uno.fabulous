...
module ViewConverters =        
    ...
    let convertFabulousImageToXamarinFormsImageSource (v: InputTypes.Image) =
        ...
        | Bytes bytes -> ImageSource.op_Implicit (new MemoryStream(bytes))
    ...
    let convertFabulousDimensionToXamarinFormsRowDefinition (v: InputTypes.Dimension array) =
        let rows =
            Array.map (fun vi ->
                match vi with
                | Auto -> let rd = RowDefinition() in rd.Height <- GridLength.Auto; rd
                | Star -> let rd = RowDefinition() in rd.Height <- GridLength(1., GridUnitType.Star); rd
                | Stars value -> let rd = RowDefinition() in rd.Height <- GridLength(value, GridUnitType.Star); rd
                | Absolute value -> let rd = RowDefinition() in rd.Height <- GridLength(value, GridUnitType.Pixel); rd
            ) v
        
        let collection = RowDefinitionCollection()
    ...
    let convertFabulousDimensionToXamarinFormsColumnDefinition (v: InputTypes.Dimension array) =
        let columns =
            Array.map (fun vi ->
                match vi with
                | Auto -> let rd = ColumnDefinition() in rd.Width <- GridLength.Auto; rd
                | Star -> let rd = ColumnDefinition() in rd.Width <- GridLength(1., GridUnitType.Star); rd
                | Stars value -> let rd = ColumnDefinition() in rd.Width <- GridLength(value, GridUnitType.Star); rd
                | Absolute value -> let rd = ColumnDefinition() in rd.Width <- GridLength(value, GridUnitType.Pixel); rd
            ) v
        
        let collection = ColumnDefinitionCollection()
