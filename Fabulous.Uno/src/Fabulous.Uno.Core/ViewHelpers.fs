...
module ViewHelpers =
    ...
    let debounce<'T> =
        ...
        fun (timeout: int) (fn: 'T -> unit) value ->
            ...
            timer.Interval <- TimeSpan.FromMilliseconds(float timeout)
    ...
    type UIElement with
        ...
        static member OnCreate (contents : ViewElement, onCreate: (obj -> unit)) =
            UIElement.Stateful (init = (fun () -> ()), contents = (fun _ -> contents), onCreate = (fun _ obj -> onCreate obj))
        ...
        static member WithInternalModel(init: (unit -> 'InternalModel), 
            ...
            UIElement.Stateful (init = (fun () -> ref (init ())), contents = (fun state -> view state.Value (internalDispatch state)))
